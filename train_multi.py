...
def train_mlp_wgan_gp(real_feats, device, epochs=1000, batch_size=64,
    ...
    # 打印模型结构
    print(f"[GAN] {sum(p.numel() for p in G.parameters()):,}")
    print(f"[GAN] {sum(p.numel() for p in D.parameters()):,}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 200 == 0 or epoch == 1:
            print(
                f"[GAN Epoch {epoch}/{epochs}] D_loss={loss_D.item():.4f}, G_loss={loss_G.item() if epoch % 5 == 0 else 'N/A'}")
...
def compute_metrics(all_logits, all_targets):
    ...
    for cls in unique_classes:
        ...
        if np.sum(cls_mask) > 0:
            cls_acc = accuracy_score(all_targets[cls_mask], preds[cls_mask])
        else:
            cls_acc = 0.0
...
def main():
    ...
    ap.add_argument("--train", default="outputs_time/multi_esm2_encode/train/embeddings.csv")
    ap.add_argument("--dev", default="outputs_time/multi_esm2_encode/valid/embeddings.csv")
    ap.add_argument("--test", default="outputs_time/multi_esm2_encode/test/embeddings.csv")
    ap.add_argument("--label-col", default="label")
    ap.add_argument("--feat-prefix", default="d")
    ap.add_argument("--epochs", type=int, default=20)
    ap.add_argument("--batch-size", type=int, default=64)
    ap.add_argument("--device", default="cuda")
    ap.add_argument("--save-dir", default="./spilt_by_smi_multi_90")

    ap.add_argument("--use-wgan", action="store_true", help="WGAN-GP")
    ap.add_argument("--z-dim", type=int, default=4)
    ap.add_argument("--wgan-epochs", type=int, default=3000)
    ap.add_argument("--wgan-batch", type=int, default=16)
    ap.add_argument("--k-classes", type=int, default=2)
    args = ap.parse_args()
    ...
    train_loader = DataLoader(InlineEmbeddingDataset(train_df), batch_size=args.batch_size, shuffle=True,
                              collate_fn=collate_pad)
    dev_loader = DataLoader(InlineEmbeddingDataset(dev_df), batch_size=args.batch_size, shuffle=False,
                            collate_fn=collate_pad)
    test_loader = DataLoader(InlineEmbeddingDataset(test_df), batch_size=args.batch_size, shuffle=False,
                             collate_fn=collate_pad)
    ...
    optimizer = torch.optim.AdamW(classifier.parameters(), lr=2e-3)

    best_f1, best_path = -1, os.path.join(args.save_dir, "best.pt")
    for epoch in range(1, args.epochs + 1):
        tr_loss, tr_acc, tr_f1, tr_f1_per_class, tr_acc_per_class, _, _, _ = run_epoch(classifier, train_loader,
                                                                                       criterion, optimizer, device)
        dv_loss, dv_acc, dv_f1, dv_f1_per_class, dv_acc_per_class, _, _, _ = run_epoch(classifier, dev_loader,
    ...
    te_loss, te_acc, te_f1, te_f1_per_class, te_acc_per_class, te_report, te_cm, te_mcc = run_epoch(classifier,
                                                                                                    test_loader,
                                                                                                    criterion, None,
                                                                                                    device)
